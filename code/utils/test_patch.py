...
def var_all_case_base_out(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
        with open('../data/LA/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["../data/LA/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
        ...
        with open('../data/Pancreas/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["../data/Pancreas/Pancreas_h5/" + item.replace('\n', '') + "_norm.h5" for item in image_list]#[:2]
        # image_list = ["../data/Pancreas/Pancreas_h5/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    ...
                    y = torch.softmax(y_logit[0], dim=1)
                y = y.cpu().data.numpy()
                y = y[0, :, :, :, :]  # y = y[0,1,:,:,:]   #
